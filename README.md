...
- **Transparent Background / Server-side Matting**: new `transparent_bg: bool` field in `init` (or env `FLASHHEAD_MATTING=1`). When enabled, each frame is matted in real time with MODNet (6.5M lightweight model, default) or RVM (Robust Video Matting, fallback) and encoded as **WebP (RGBA with alpha)** instead of JPEG; `frames_meta` carries a new `fmt` field (`jpeg` / `webp`) so clients know the encoding. The matting model is downloaded automatically on first use and falls back to JPEG on failure. Switch model via `FLASHHEAD_MATTING_MODEL` (`modnet` / `rvm`).
