...
|-------|------|---------|-------------|
| `cond_image` | string | required | Base64-encoded JPEG/PNG, or local file path if `cond_is_path=true` |
...
**Then:** Binary WebSocket frame — raw `uint8` bytes of shape `(frames_count, height, width, 3)` in C-contiguous RGB order.
